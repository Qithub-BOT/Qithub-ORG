...
Linuxのwpa_supplicant v2.6も、4ウェイ-ハンドシェイクでオールゼロ暗号鍵をインストールすると脆弱です。これはJohn A. Van Boxtelによって発見されました。その結果、6.0より新しいすべてのAndroidバージョンも攻撃の影響を受けるため、すべてゼロの暗号化キーをインストールすることに騙される可能性があります。新しい攻撃は、再送信されたメッセージ3を犠牲者に転送する前に元のメッセージ1で使用されたのと同じANonceを持つ偽造メッセージ1を注入することによって機能します。
