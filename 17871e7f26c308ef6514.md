...
パケットが解読（そしておそらく偽造）されるかどうかは、ハンドシェイクが攻撃されているかに依ります。単純に、4-ウェイハンドシェイクが攻撃されているなら、我々はクライアントから送られてきたパケットを解読（偽造）できます。高速ローミング機能のハンドシェイクが攻撃されているなら、我々はクライアントに送るパケットを解読（偽造）できます。
...
我々の攻撃は、**Wi-Fiネットワークのパスワードを復元していない**し、また、4-ウェイハンドシェイク中に通用したばかりの暗号化キー（のいかなる部分）も復元していないことに注意してください。
