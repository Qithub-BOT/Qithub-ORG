...
概念実証として、私たちは Android スマートフォンに対して「Key Reinstallation」攻撃を実施しました。このデモンストレーションでは、攻撃者は被害者が送信するすべてのデータを復号化することができます。攻撃者にとって、これは簡単に達成できることです。なぜなら、私たちの「Key Reinstallation」攻撃は、Linux や Android 6.0 以上に対して桁外れに破壊的であるからです。これは Android と Linux が、暗号化キーがすべてゼロのものをインストールもしくは再インストールされても騙される可能性があるからです（詳細は下記を参照）。他のデバイスを攻撃する場合、すべてのパケットを解読することは難しいのですが、多数のパケットを解読することは可能です。いずれの場合でも、次のデモンストレーションでは「Key Reinstallation」攻撃を実行する際に保護された Wi-Fi ネットワークに対して、攻撃者が取得できる情報の種類をハイライトしています。 https://www.youtube.com/watch?time_continue=3&v=Oh4WURZoR98
...
私たちの攻撃は（電子メールアドレスやパスワードなどの）ログイン情報の復元に限定されません。一般的に、被害者が送信するデータまたは情報はすべて復号化できます。さらに、使用されているデバイスやネットワークの設定によっては、被害者に送信されたデータ（たとえばウェブサイトのコンテンツ）を復号化することもできます。ウェブサイトやアプリでは HTTPS をさらなる保護層として使用することがありますが、この二重保護さえも憂慮すべき状況でバイパスされる可能性があると警告します。たとえば、非ブラウザ系のソフトウェア、Apple の iOS と OS X 、Android アプリ、そして Android アプリ、銀行のアプリ、そしてさらに VPN アプリでさえも以前はバイパスされていました。
...
私たちの主な攻撃は、WPA2プロトコルの4ウェイハンドシェイクに対するものです。このハンドシェイクは、クライアントが保護されたWi-Fiネットワークに参加したいときに実行され、クライアントとアクセスポイントの両方が正しい資格情報（ネットワークの事前共有パスワードなど）を所有していることを確認するために使用されます。同時に、4ウェイハンドシェイクは、以降のすべてのトラフィックを暗号化するために使用される新鮮な暗号化キーについても交渉します。現在、近代的に保護されているすべてのWi-Fiネットワークでは、4方向ハンドシェイクが使用されています。これは、これらのネットワークのすべてが攻撃の影響を受けることを意味します。たとえば、この攻撃は、個人および企業のWi-Fiネットワーク、古いWPAおよび最新のWPA2標準、さらにはAESのみを使用するネットワークに対しても機能します。 **WPA2に対する私たちの攻撃は、キー再インストール攻撃（KRACK）と呼ばれる新しい技術を使用しています**。

### キーの再インストール攻撃：高度な説明

> In a key reinstallation attack, the adversary tricks a victim into reinstalling an already-in-use key. This is achieved by manipulating and replaying cryptographic handshake messages. When the victim reinstalls the key, associated parameters such as the incremental transmit packet number (i.e. nonce) and receive packet number (i.e. replay counter) are reset to their initial value. Essentially, to guarantee security, a key should only be installed and used once. Unfortunately, we found this is not guaranteed by the WPA2 protocol. By manipulating cryptographic handshakes, we can abuse this weakness in practice.

キーの再インストール攻撃では、敵が既に使用中のキーを再インストールするように犠牲者を騙します。これは、**暗号化ハンドシェイクメッセージを操作して再生することによって達成される**。犠牲者が鍵を再インストールすると、増分送信パケット数（すなわち、ノンス）および受信パケット数（すなわち、リプレイカウンタ）などの関連パラメータがそれらの初期値にリセットされる。基本的には、セキュリティを保証するために、鍵は一度だけインストールして使用する必要があります。残念ながら、WPA2プロトコルでは保証されていません。暗号化ハンドシェイクを操作することにより、実際にこの弱点を悪用する可能性があります。

### キー再インストール攻撃：4ウェイハンドシェイクに対する具体的な例
...
[研究論文の紹介](https://www.krackattacks.com/#paper)に記載されているように重要な再インストール攻撃の背後にあるアイデアは、以下のように要約できます。クライアントがネットワークに参加すると、4ウェイハンドシェイクが実行されて新しい暗号化キーがネゴシエートされます。4ウェイハンドシェイクのメッセージ3を受信した後、このキーをインストールします。キーがインストールされると、暗号化プロトコルを使用して通常のデータフレームを暗号化するために使用されます。ただし、メッセージが失われたり削除される可能性があるため、アクセスポイント（AP）は、確認応答として適切な応答を受信しなかった場合、メッセージ3を再送信します。その結果、クライアントはメッセージ3を複数回受信する可能性があります。このメッセージを受信するたびに、同じ暗号化キーを再インストールし、暗号化プロトコルで使用される増分送信パケット番号（nonce）と受信再生カウンタをリセットします。私たちはそれを示します攻撃者は、**4ウェイハンドシェイクのメッセージ3の再送信を収集して再生することによって、これらのナンスのリセットを強制することができます**。このようにナンスの再使用を強制することによって、暗号化プロトコルを攻撃することができ、例えば、パケットを再生、復号化、および/または偽造することができる。同じ技法を使用して、グループ鍵、PeerKey、TDLS、および高速BSS移行ハンドシェイクを攻撃することもできます。
...
私たちの意見では、最も広く普及している影響力のある攻撃は、4ウェイハンドシェイクに対する主要な再インストール攻撃です。我々はこの判断を2つの観察に基づいて行う。まず、私たち自身の研究の最中に、ほとんどのクライアントがその影響を受けていることがわかりました。第2に、敵対者はこの攻撃を使用してクライアントから送信されたパケットを復号化し、パスワードやクッキーなどの機密情報を傍受することができます。キー再インストール攻撃により、送信ナンス（パケット番号または初期化ベクトルとも呼ばれることもある）がゼロにリセットされるため、パケットの復号化が可能です。その結果、**過去にすでに使用されているnonce値と同じ暗号化キーが使用されます**。これにより、WPA2のすべての暗号化プロトコルで[キーストリーム](https://en.wikipedia.org/wiki/Keystream)が再利用されますパケットを暗号化するとき キーストリームを再利用するメッセージに既知のコンテンツがある場合は、使用されているキーストリームを導出することは簡単です。このキーストリームを使用して、同じナンスでメッセージを復号化することができます。既知のコンテンツがない場合は、パケットを復号化することは難しくなりますが、いくつかのケースでは可能です（たとえば[英語のテキストは引き続き復号化できます](https://crypto.stackexchange.com/a/2250)）。 実際には、既知の内容のパケットを見つけることは問題ではないので、どのパケットも復号化できると仮定する必要があります。
...
パケットが解読される（そして偽造される）方向は攻撃されるハンドシェイクに依存する。シンプルに、4-wayハンドシェイクを攻撃すると、クライアントから送信されたパケットを復号化（および偽造）することができます。高速BSSの移行（FT）握手を攻撃するとき、私たちは解読（および偽造）することができます送信されたパケットに向けてクライアント。最後に、攻撃の大半はユニキャスト、ブロードキャスト、およびマルチキャストフレームの再生を可能にします。詳細は、[研究論文](https://www.krackattacks.com/#paper)のセクション6を参照してください。
...
### 攻撃者がman-in-the-middle攻撃を行うことができる場合、なぜ彼はすべてのデータを復号化できませんか？
...
デモンストレーションで述べたように、攻撃者は最初に被害者と実際のWi-Fiネットワーク（チャネルベースのMitMポジションと呼ばれる）の中間者（MitM）ポジションを取得します。しかし、このMitMの位置では、攻撃者はパケットを解読することができません！この位置では、攻撃者は暗号化されたパケットを確実に遅延、ブロック、再生することができます。したがって、攻撃のこの時点で、彼または彼女はまだパケットを解読することができません。代わりに、パケットを確実に遅延させてブロックする機能を使用して、鍵の再インストール攻撃を実行します。鍵の再インストール攻撃を実行した後、パケットを復号化することができます。
