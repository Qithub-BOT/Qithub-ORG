...
概念実証として、私たちはAndroidスマートフォンに対する主要な再インストール攻撃を実行しました。このデモンストレーションでは、攻撃者は被害者が送信するすべてのデータを復号化することができます。攻撃者にとって、これは簡単に達成できます。なぜなら、私たちの主要な再インストール攻撃は、LinuxやAndroid 6.0以上に非常に壊滅的なものであるからです。これは、AndroidとLinuxがすべてゼロの暗号化キーをインストールすることに騙される可能性があるからです（詳細は下記を参照）。他のデバイスを攻撃する場合、すべてのパケットを解読することは難しくなりますが、多数のパケットを解読することは可能です。いずれの場合でも、次のデモンストレーションでは、保護されたWi-Fiネットワークに対してキー再インストール攻撃を実行する際に、攻撃者が取得できる情報の種類が強調されています。 https://www.youtube.com/watch?time_continue=3&v=Oh4WURZoR98
...
私たちの攻撃はログイン資格情報（電子メールアドレスとパスワード）の回復に限定されません。一般に、被害者が送信するデータまたは情報はすべて復号化できます。さらに、使用されているデバイスやネットワークの設定によっては、被害者に送信されたデータ（たとえばウェブサイトのコンテンツ）を復号化することもできます。ウェブサイトやアプリではHTTPSを追加の保護層として使用することがありますが、この余分な保護は憂慮すべき状況でバイパスされる可能性があると警告しています。 たとえば、HTTPSは、以前にバイパスされたブラウザ以外のソフトウェアでは、AppleのiOSとOS Xで、Androidアプリで、再びAndroidアプリでは、銀行のアプリ、そしてVPNアプリでさえも。
