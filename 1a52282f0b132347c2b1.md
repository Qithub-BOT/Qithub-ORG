...
### プルリクエストの処理 ([items: #19](https://github.com/Qithub-BOT/items/issues/19))
...
マージは、 **「プルリクエストを上げた本人以外のマージ権限保持者」** によって、 **「プルリクエストのコメントに記述されている変更が達成されており、記述にない変更がない」** コミットに対して行われます。

##### マージされない例

>例1）記事 yyyyyyyyyyy.md だけでなく 記事 zzzzzzzzzzz.md も同一ブランチで編集している

>例2）プルリクエストのコメントに「投票集計機能を追加しました。」と書かれているのに、同じブランチで定時トゥートも変更されている

#### マージを行う人

マージは、 **「プルリクエストを上げた本人以外のマージ権限保持者」** によってマージされます。

#### マージ後のブランチの処理

ブランチはマージ後、**マージを行った人**が責任をもって破棄します。

何らかの理由で破棄しない場合は、その理由をプルリクエストのコメントに残してください。
