...
- [Organization の運営ルール](#Organization-の運営ルール)
  ...
  - [Organization の運用ルールの参照先](#Organization-の運用ルールの参照先)
  - [プロジェクト全体に関する相談窓口はどこ？](#プロジェクト全体に関する相談窓口はどこ？)
  ...
  - [issue の最終的な結論をスレッドの頭に取りまとめる](#issue-の最終的な結論をスレッドの頭に取りまとめる)
    - [マージを行う人](#マージを行う人)
    - [マージを行う人](#マージを行う人)
  - [Qiitaコラボ記事「Organization 全体で使われる用語集」](#Qiitaコラボ記事「Organization-全体で使われる用語集」)
  - [Qiitaコラボ記事「この BOT の機能について」記事ID](#Qiitaコラボ記事「この-BOT-の機能について」記事ID)
...
- [Qithub の仕様](#Qithub-の仕様)
  - [BOT開発におけるマージの基準とデプロイ](#BOT開発におけるマージの基準とデプロイ)
    - [基本ルール](#基本ルール)
      ...
      - [デプロイに関して](#デプロイに関して)
  - [BOTの実行間隔の仕様](#BOTの実行間隔の仕様)
  - [BOTによる新規Qiita記事の作成と同期の仕様](#BOTによる新規Qiita記事の作成と同期の仕様)
...
## Qiitaコラボ記事 目次 ([items #42](https://github.com/Qithub-BOT/items/issues/42))
...
ラグインには issueXX という名前を使わず、テンプレートをドキュメントに指示し、それに沿わせる
例）<動詞>-<対象>
ファイル名として、エントリポイント（main関数的な、プラグインの起点となるファイル）には main.<拡張子> を指定する。
例1）main.php
例2）main.py
- 「plugins」内のプラグイン名は issue番号 でなく「＜DO＞-＜WHAT＞」#### 対Qiitadonの必記事を作成した旨を ユーザA のトゥート7須機能
...
## BOT のプラグイン仕様 ([scripts #16](https://github.com/Qithub-BOT/scripts/issues/16))
